...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "SELECT cotCode FROM accounts WHERE id = $userId";
    ...
    if ($result) {
        ...
        if ($row) {
            ...
            $savedCOT = $row['cotCode'];
          

            // Check if any of the code fields are empty
            if (empty($cotCode)) {
                $errorMsg .= 'Cot code is required for this transaction to be completed successfully.';
            } else {
...
<h1>Please enter the COT code to continue</h1>
</div>
<div id="formdiv">
<form method="POST" action=""  id="transferFormz">
        <label for="cotCode">COT Code:</label><br>
        <input type="text" id="cotCode" name="cotCode" required style="width:200px"><br>
